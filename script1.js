...
function elementByIdEventListener(n, e, t="click") {
    const o = document.getElementById(n);
    isFunction(e) ? null !== o && o.addEventListener(t, function() {
        e()
    }) : console.error("call back function not defined")
}
function isFunction(n) {
    return typeof n == typeof Function
}
...
!function(a) {
    ...
    a(function() {
        ...
        a("body").on("click.dropdown", ".dropdown form", function(a) {
            a.stopPropagation()
...
!function(a) {
    ...
    function e(b) {
        ...
        if (this.isShown && this.options.backdrop) {
            ...
            a.support.transition && this.$element.hasClass("fade") ? this.$backdrop.one(a.support.transition.end, a.proxy(f, this)) : f.call(this)) : b && b()
    }
    function f() {
...
!function(a) {
    ...
    b.prototype = {
        ...
        lookup: function(b) {
            ...
            this.query ? (d = a.grep(this.source, function(a) {
                return c.matcher(a)
            }),
            d = this.sorter(d),
            d.length ? this.render(d.slice(0, this.options.items)).show() : this.shown ? this.hide() : this) : this.shown ? this.hide() : this
        },
        matcher: function(a) {
...
elementByIdEventListener('TransliterationHelpMobile', function() {
    let transliterationHelpMenuMobile = document.getElementById('TransliterationHelpMenuMobile');

    if (transliterationHelpMenuMobile !== null) {
        if (transliterationHelpMenuMobile.style.display === 'none') {
            transliterationHelpMenuMobile.style.display = 'block';
        } else {
            transliterationHelpMenuMobile.style.display = 'none';
        }
    }
});

elementByIdEventListener('TransliterationHelpOnSourceMobile', function() {
    let transliterationHelpMenuMobile = document.getElementById('TransliterationHelpMenuOnSourceMobile');
...
document.getElementById("SaveTextAsFile").addEventListener("click", function(e) {
    let t = document.getElementsByClassName("downloadable")[0]
      , n = window.location.hostname.split(".")[1]
      , o = "" !== n ? n + ".txt" : "dowloaded_typing_text.txt";
    "" !== t && (console.error("Sorry, the content of the textarea could not be downloaded!"),
    e.preventDefault()),
    saveTextAsFile(t.value, o)
}),
document.getElementById("CopyTool").addEventListener("click", function(e) {
    copyToClipboard(document.getElementsByClassName("copiable")[0])
});
// copy

// Toogle the menu on clicking the close button
elementByIdEventListener('CloseTransliterationHelpMobile', function() {
    document.getElementById('TransliterationHelpMenuMobile').style.display = 'none';
    document.querySelector('.textarea-footer-toolbar-item svg').style.fill = '#666666';
});

elementByIdEventListener('CloseTransliterationHelpOnSourceMobile', function() {
    document.getElementById('TransliterationHelpMenuOnSourceMobile').style.display = 'none';
    document.querySelector('.textarea-footer-toolbar-item-on-source svg').style.fill = '#666666';
});

let activateTransliterationHelpMobile = !1
  , activateTransliterationHelpOnSourceMobile = !1;
if (elementByIdEventListener("TransliterationHelpMobile", function() {
    !0 === (activateTransliterationHelpMobile = !activateTransliterationHelpMobile) && _gaq.push(["_trackEvent", "TransliterationHelpButtonClickedMobile", "TransliterationHelpButtonClickedMobile-on-page-" + getPathName(), "TransliterationHelpButtonClickedMobile", , !0])
}),
elementByIdEventListener("CloseTransliterationHelpMobile", function() {
    activateTransliterationHelpMobile = !1
}),
elementByIdEventListener("TransliterationHelpOnSourceMobile", function() {
    !0 === (activateTransliterationHelpOnSourceMobile = !activateTransliterationHelpOnSourceMobile) && _gaq.push(["_trackEvent", "TransliterationHelpButtonOnSourceClicked", "TransliterationHelpButtonClickedOnSource-on-page-" + getPathName(), "TransliterationHelpButtonOnSourceClicked", , !0])
}),
elementByIdEventListener("CloseTransliterationHelpOnSource", function() {
    activateTransliterationHelpOnSourceMobile = !1
}),
elementByIdEventListener("SaveTextAsFile", function() {
    _gaq.push(["_trackEvent", "TextAreaContentDownloaded", "TextAreaContentDownloaded-on-page-" + getPathName(), "TextAreaContentDownloaded", , !0])
}),
elementByIdEventListener("CopyTool", function() {
    _gaq.push(["_trackEvent", "TextAreaContentCopied", "TextAreaContentCopied-on-page-" + getPathName(), "TextAreaContentCopied", , !0])
}),
elementByIdEventListener("EditTargetTextarea", function() {
    _gaq.push(["_trackEvent", "EditTargetTextAreaClicked", "EditTargetTextAreaClicked-on-page-" + getPathName(), "EditTargetTextAreaClicked", , !0])
}),
"function" != typeof getPathName)
    function getPathName() {
        if ("undefined" == typeof window || void 0 === window.location || void 0 === window.location.pathname)
            return "invalid-path";
        const e = window.location.pathname.split("?")[0];
        return "/" === e ? "homepage" : e.slice(1)
    }
...
var cleanPathName = pathName.replace("/", "_");
if (typeof (Storage) !== "undefined") {
    console.log('LocalStorage Supported.');
    var storageKey = 'storageFor' + domainName + cleanPathName;
    var storableTextarea = document.getElementsByClassName('storable')[0];
    var storage = new LocalStorage();
    storableTextarea.addEventListener('keyup', function(e) {
        var content = this.value;
        if (e.keyCode == 0 || e.keyCode == 32 || e.keyCode == 8 || e.keyCode == 13 || e.keyCode == 46) {
            storage.write(storageKey, content)
        }
    });
    if (storage.read(storageKey) != null) {
        storableTextarea.innerHTML = storage.read(storageKey)
    }
} else {
    console.log('Local Storage Not Support. Defaulting Back to Cookie.');
    var typingCookieName = 'saveTypingFor' + domainName + cleanPathName;
    $('#languageTextarea, #languageTextareaNew').keyup(function(e) {
        if (e.keyCode == 0 || e.keyCode == 32 || e.keyCode == 8 || e.keyCode == 13 || e.keyCode == 46) {
            var textareaContent = $('#languageTextarea, #languageTextareaNew').val();
            createCookie(typingCookieName, textareaContent, 7)
        }
    });
    if (readCookie(typingCookieName) != null) {
        $('#languageTextarea, #languageTextareaNew').val(readCookie(typingCookieName).trim())
    }
}
let rotators = document.getElementsByClassName("rotator");
for (let t = 0; t < rotators.length; t++) {
    let e = rotators[t]
      , a = 15e3;
    e.style.display = "block",
    void 0 !== e.dataset.rotateTransition && (a = 1e3 * parseInt(e.dataset.rotateTransition)),
    $(e).cycle({
        fx: "fade",
        timeout: a
    })
    ...
    20: function(t, e, n) {
        ...
        $(document).ready(function() {
            ...
            $("#input_tool").on("keyup", function(t) {
                8 == t.which && ($("#input_tool_helper").empty(),
                n ? $("#input_tool_helper").append("<em>English mode enabled. Press (CTRL + G) to switch back.</em>") : $("#input_tool_helper").append("<em>.....</em>"))
            }),
            $("#input_tool").click(function(n) {
                for (var l = [" ", "\n", "\r", "\t"], u = $(this).val(), p = $(this)[0].selectionStart, r = $(this)[0].selectionEnd; p > 0 && -1 == l.indexOf(u[p]); )
                    --p;
                for (++p; r < u.length && -1 == l.indexOf(u[r]); )
                    ++r;
            ...
            $("#input_tool").on("keyup", function(l) {
                if (n)
                    return $("#input_tool_helper").empty(),
                    void $("#input_tool_helper").append("<em>English mode enabled. Press (CTRL + G) to switch back.</em>");
                ...
                t.onreadystatechange = function() {
                    if (t.readyState == t.DONE)
                        if (200 == t.status) {
                            ...
                            l("Rejected status code: " + t.status)
                }
                ,
                t.onerror = function() {
